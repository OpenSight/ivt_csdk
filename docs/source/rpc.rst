    ...
    "channels": [
      {
        ...
        "alarm": <可选，整数；当前报警状态flag，每一位（从0开始计数）对应一种报警类型，当相应位为1时，标示该类型的报警被触发；第2位，外部报警；第3位，移动侦测；当该域不存在时表示当前没有报警>
      }
    ]
  }
...
AlarmCrossLineDetectConfig
++++++++++++++

IVC可以通过该方法修改摄像头的拌网/拌线检测相关的参数
...
参数： ::

  {
    ...
    "end": <可选，整数；每天自动撤防时间，从00：00开始算起的秒数；当不存在时为86400，即第二天的0点0分>,
    "sensitivity": <必填，整数；灵敏度，0-100，0为最不灵敏，100为最灵敏>,
    "delay": <必填，整数；抖动时间，单位秒，侦测到拌网/拌线后，在指定的delay时间内没有再次侦测到，则上报报警结束>
  }
